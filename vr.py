...
class VuerPreprocessor:
    def __init__(self, manus_receiver=None):
        ...
        plt.ion()
        self.fig_debug = plt.figure(figsize=(6, 6))
        self.ax_debug = self.fig_debug.add_subplot(111, projection='3d')
        print("Debug Window Initialized.")


    def update_debug_plot(self, head, left, right):
        # Apple Vision Pro
        # +x: right
        # -x: left
        # +y: top
        ...
        # 1. 设置固定范围
        self.ax_debug.set_xlim([-1, 1])
        self.ax_debug.set_ylim([-1, 1])
        self.ax_debug.set_zlim([0, 2])
        
        # 放大全局坐标轴标签字体
        self.ax_debug.set_xlabel('X (Side)', fontsize=12, fontweight='bold')
        self.ax_debug.set_ylabel('Y (Front)', fontsize=12, fontweight='bold')
        self.ax_debug.set_zlabel('Z (Up)', fontsize=12, fontweight='bold')

        # 定义一个内部辅助函数，画出显著的 RGB 局部坐标轴
        def draw_heavy_axis(mat, length=0.3):  # length 增加到 0.3 米
            pos = mat[:3, 3]
            # 局部 X, Y, Z 方向向量
            x_dir = mat[:3, 0]
            y_dir = mat[:3, 1]
            ...
            # 绘制加粗的 RGB 轴
            # X轴-红色, Y轴-绿色, Z轴-蓝色
            params = {'length': length, 'linewidth': 4, 'arrow_length_ratio': 0.3}
            self.ax_debug.quiver(pos[0], pos[1], pos[2], x_dir[0], x_dir[1], x_dir[2], color='red', **params)
            self.ax_debug.quiver(pos[0], pos[1], pos[2], y_dir[0], y_dir[1], y_dir[2], color='green', **params)
            self.ax_debug.quiver(pos[0], pos[1], pos[2], z_dir[0], z_dir[1], z_dir[2], color='blue', **params)
        ...
        # 3. 绘制巨大的局部坐标系 (RGB=XYZ)
        draw_heavy_axis(head, length=0.3)
        draw_heavy_axis(left, length=0.25)
        draw_heavy_axis(right, length=0.25)

        # 4. 绘制巨大的散点 (s=300)
        self.ax_debug.scatter(h_xyz[0], h_xyz[1], h_xyz[2], c='red', s=300, edgecolors='black', label='Head')
        self.ax_debug.scatter(l_xyz[0], l_xyz[1], l_xyz[2], c='blue', s=250, edgecolors='black', label='L-Wrist')
        self.ax_debug.scatter(r_xyz[0], r_xyz[1], r_xyz[2], c='green', s=250, edgecolors='black', label='R-Wrist')

        # 5. 绘制醒目的坐标文字
        # 使用 backgroundcolor 让文字在 3D 空间中不被线条遮挡
        text_style = {'fontsize': 11, 'fontweight': 'bold', 'bbox': dict(facecolor='white', alpha=0.7, edgecolor='none')}
        offset = 0.08
        self.ax_debug.text(h_xyz[0]+offset, h_xyz[1], h_xyz[2], f"H: {h_xyz[0]:.2f}, {h_xyz[1]:.2f}, {h_xyz[2]:.2f}", **text_style)
        self.ax_debug.text(l_xyz[0]+offset, l_xyz[1], l_xyz[2], f"L: {l_xyz[0]:.2f}, {l_xyz[1]:.2f}, {l_xyz[2]:.2f}", **text_style)
        self.ax_debug.text(r_xyz[0]+offset, r_xyz[1], r_xyz[2], f"R: {r_xyz[0]:.2f}, {r_xyz[1]:.2f}, {r_xyz[2]:.2f}", **text_style)

        # 6. 加粗连线 (骨架)
        self.ax_debug.plot([h_xyz[0], l_xyz[0]], [h_xyz[1], l_xyz[1]], [h_xyz[2], l_xyz[2]], 'k--', linewidth=2, alpha=0.4)
        self.ax_debug.plot([h_xyz[0], r_xyz[0]], [h_xyz[1], r_xyz[1]], [h_xyz[2], r_xyz[2]], 'k--', linewidth=2, alpha=0.4)

        self.ax_debug.set_title("Bigger & Clearer Pose Debug (RGB=XYZ)", fontsize=14)
        
        # 调整图例大小
        self.ax_debug.legend(loc='upper left', prop={'size': 10})
    ...
    def process(self, tv):
        ...
        self.update_debug_plot(self.vuer_head_mat, self.vuer_left_wrist_mat, self.vuer_right_wrist_mat)
